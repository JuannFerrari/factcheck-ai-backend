...
from app.domain.models import Source, HybridSearchResult, VectorSearchResult
...
class HybridSearchService:
    ...
    async def search_claim(
        ...
        try:
            ...
            if use_vector_cache and settings.enable_vector_search:
                ...
                if exact_match:
                    logger.info(
                        f"Found exact match in vector database for: {claim[:50]}..."
                    )
            ...
            if use_vector_cache and settings.enable_vector_search:
                ...
                if exact_match:
                    exact_vector_result = VectorSearchResult(
                        record=exact_match, similarity_score=1.0, is_exact_match=True
                    )
                    vector_results.insert(0, exact_vector_result)

                if vector_results:
                    logger.info(
                        f"Found {len(vector_results)} similar claims in vector database"
                    )
                    used_vector_cache = True
            ...
            try:
                web_results = await web_search_service.search_claim(
                    claim, num_web_results
                )
                return HybridSearchResult(
                    vector_results=[],
                    web_results=web_results,
    ...
    async def _combine_and_deduplicate_sources(
        self, vector_results: List[VectorSearchResult], web_results: List[Source]
    ) -> List[Source]:
        ...
        for vector_result in vector_results:
            if vector_result.similarity_score >= 0.9:  # High confidence matches
                ...
                sources_data = (
                    json.loads(vector_result.record.sources_json)
                    if vector_result.record.sources_json
                    else []
                )
        ...
        logger.info(
            f"Combined {len(combined_sources)} unique sources from vector and web search"
        )
