...
class VectorDatabaseService:
    ...
    async def store_fact_check(
        ...
        try:
            ...
            normalized_claim = await embedding_service.normalize_text_for_embedding(claim)
            ...
            if len(embedding) != EMBEDDING_DIMENSIONS:
                logger.error(f"Invalid embedding dimensions: {len(embedding)}, expected {EMBEDDING_DIMENSIONS}")
                return None
    ...
    def _normalize_sources(self, sources: List[Union[Source, Dict[str, Any]]]) -> List[Dict[str, Any]]:
    ...
    async def find_similar_claims(
        ...
        try:
            ...
            try:
                async with asyncio.timeout(30.0):
                    query_embedding = await embedding_service.get_embedding(normalized_claim)
            except asyncio.TimeoutError:
            ...
            stmt = select(
                ...
                FactCheckRecordDB.claim_embedding.is_not(None)
            ).order_by(
                text("claim_embedding <-> :embedding")
            ...
            for row in rows:
                if row.claim_embedding is not None:
                    ...
                    if similarity >= similarity_threshold:
                        ...
                        is_exact_match = claim.lower().strip() == row.claim.lower().strip()
            ...
            logger.info(f"Found {len(vector_results)} similar claims for: {claim[:50]}...")
    ...
    async def get_recent_fact_checks(
        ...
        try:
            stmt = select(FactCheckRecordDB).order_by(
                FactCheckRecordDB.created_at.desc()
            ).limit(limit)
    ...
    async def _db_to_domain_model(self, db_record: FactCheckRecordDB) -> FactCheckRecord:
        ...
        try:
            ...
            if db_record.sources_json:
                try:
                    sources_data = json.loads(db_record.sources_json)
                except json.JSONDecodeError as e:
                    logger.warning(f"Failed to parse sources JSON for record {db_record.id}: {e}")
                    sources_data = []
    ...
    async def cleanup_old_records(self, session: AsyncSession, days: int = 90) -> int:
        ...
        try:
            stmt = select(FactCheckRecordDB.id).where(
                FactCheckRecordDB.created_at < func.now() - text(f"INTERVAL '{days} days'")
            )
            result = await session.execute(stmt)
