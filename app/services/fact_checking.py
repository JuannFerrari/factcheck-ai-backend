...
async def call_model(claim: str, sources_text: str, similar_claims_context: str = "") -> str:
...
async def store_fact_check_background(
    ...
    if verdict in ["Rejected", "Unclear"] and confidence < 50:
        logger.info(f"Background: Skipping storage for low-confidence {verdict} verdict")
        return
    ...
    try:
        async with asyncio.timeout(CACHE_TIMEOUT):
            async with AsyncSessionLocal() as session:
                ...
                if stored_record:
                    logger.info(f"Background: Stored fact-check in vector database with ID: {stored_record.id}")
                else:
                    logger.warning("Background: Failed to store fact-check in vector database")
...
def build_similar_claims_context(hybrid_result) -> tuple[str, bool]:
    ...
    has_contradictions = len(set(s['verdict'] for s in similar_claims)) > 1 if len(similar_claims) > 1 else False
    ...
    if similar_claims:
        ...
        for i, similar in enumerate(similar_claims, 1):
            ...
            similar_claims_context += f"{i}. Claim: '{similar['claim']}' - Verdict: {similar['verdict']} (Confidence: {similar['confidence']}%, Similarity: {similar['similarity']:.2f}){exact_marker}\n"
        if has_contradictions:
            similar_claims_context += "\n⚠️ CONTRADICTION DETECTED: Multiple similar claims have different verdicts. This suggests the topic may be disputed or requires careful investigation of which claim is actually correct.\n"
        similar_claims_context += "\nThis context may be relevant to your analysis. Pay special attention to contradictions and investigate which claim is actually correct."
...
async def fact_check_chain_logic(input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if hybrid_result.used_vector_cache:
                ...
                if hybrid_result.vector_results:
                    logger.info(f"Found {len(hybrid_result.vector_results)} similar claims")
            ...
            similar_claims_context, has_contradictions = build_similar_claims_context(hybrid_result)
    ...
    # --- 3. AI analysis ---
    try:
        analysis = await call_model(claim, format_sources(sources), similar_claims_context)
    except Exception as e:
